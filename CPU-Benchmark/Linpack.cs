...
namespace CPU_Benchmark
{
    public class Linpack
    {
        ...
        private double mflops_result_total = 0.0;
        ...
        public double MFlopsTotal
        {
            get { return mflops_result_total; }
        }
        ...
        public void RunBenchmark(int threads)
        {
            ...
            while (taskCompleteCounter < threads)
            { }

            stopwatch.Stop();
            total_time = stopwatch.Elapsed.TotalMilliseconds / 1000;

            int n = DEFAULT_PSIZE;
            double ops = (2.0e0 * (((double)n) * n * n)) / 3.0 + 2.0 * (n * n);

            mflops_result_total = ops * threads / (1.0e6 * total_time);
            mflops_result_total += 0.0005; // for rounding
            mflops_result_total = (int)(mflops_result_total * 1000);
            mflops_result_total /= 1000;
